...
export function useProviderSetup(opts?: { onSaved?: () => void }): ProviderSetupState {
  ...
  const fetchModels = async () => {
    ...
    if (!res.ok) {
      setFetchModelsError(res.error || "无法获取模型列表");
...
export function ProviderForm({
  ...
  return (
    <div>
      ...
      {/* 获取模型（cc-switch 风格）：仅对带密钥字段的自定义端点提供商生效 */}
      {(() => {
        const keyed = (info?.fields || []).some((x) => x.secret);
        ...
        return (
          <div className="mt-4">
              ...
              <div className="mt-2 max-h-48 overflow-y-auto rounded-lg border border-line bg-panel">
                {ps.models.map((m) => (
                  <div
                    ...
                    title={m.owned_by ? `所属：${m.owned_by}` : undefined}
                  >
            ...
            {ps.models && ps.models.length === 0 && (
              <p className="mt-1 text-[11.5px] text-faint">未返回任何模型。</p>
            )}
          </div>
        );
      })()}
